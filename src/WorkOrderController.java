...
public class WorkOrderController implements Initializable {
	...
	private TreeItem<String> root, masonrySupplies, bulkGoods, disposal,
			fencing, masonry, miscellaneous, pipes, vehicles_eqipm, woods,
			stones, blocks, ADS, pipe1_5, pipe2, pipe3, pipe4;
	...
	private double total_workhours;

	// controller accepts 4 parameters
	public WorkOrderController(String jobNumber, String clientName,
			String jobType, double cost) throws SQLException, Exception {
	 ...
	 */
	@FXML
	public void check(ActionEvent event) throws SQLException, ParseException,
			IOException {
		// System.out.println("Output after constructor was initialized " +
		// jobNumber);
		...
		boolean answer = ConfirmationBox.display("",
				"Are you sure to save this input");
		try {
			if (answer && !leader.equals(null) && workhours != 0
	...
	public void addWorker() throws ParseException {
		...
		try {
			if (!name.equals(null) && !timein.equals(null)
					&& !timeout.equals(null) && !name.equals("")) {
				System.out.println("add worker pressed--> " + name + ", "
						+ timein + " - " + timeout);
				...
				double amounts[] = new double[2]; // amounts[0] is cost,
													// amounts[1] is hourly
				...
				double new_cost = Math.round(TM_cost * 100.0) / 100.0; // rounding
																		// to
																		// two
																		// decimal
																		// places
				...
				System.out.println("Name of worker: " + name + ", cost= "
						+ cost + " total_daily_cost= " + actualRawCost
						+ ", work hours= " + workhours + ", total cost= "
						+ total + ", total billed amount= " + TM_cost);
				EmployeeHours worker = new EmployeeHours(name, timein, timeout,
						workhours, cost, hourly, total, totalhourly);
	...
	public void removeWorker() {
		...
		try {
			...
			actualRawCost = Math
					.round((actualRawCost - e.getHourlytotal()) * 100.0) / 100.0;
			System.out.println("Removed worker: " + e.getName()
					+ ", actual cost= " + e.getHourlytotal() + ", billed= "
					+ e.getCost());
	...
	public void addItem() throws SQLException {
		...
		String amountfield = itemAmount.getText(); // getting the amount of
													// items

		try {
			String str = tools.getSelectionModel().getSelectedItem().getValue(); // getting
																					// the
																					// name
																					// of
																					// selected
																					// item
			Tools t = connection.getTool(str); // getting the information of the
												// item by the given name

			if (str.length() != 0 && !str.equals(null)
					&& amountfield.trim().length() != 0
					&& !amountfield.equals(null)) {
				...
				dailyMaterialCost = Math
						.round((dailyMaterialCost + totalMaterialCost) * 100.0) / 100.0;
				...
				dailyTotal.setText(String.valueOf(dailyCost));
				actualRawCost = Math
						.round((actualRawCost + totalMaterialCost) * 100.0) / 100.0;

				t.setAmount(iteamAmount);
				t.setTotal(totalMaterialBilled);
				toolsList.add(t);

				System.out.println("Added item: " + t.getName() + ", amount="
						+ iteamAmount + ", cost= " + t.getCost() + ", "
						+ "billed= " + t.getBilled() + ", totalBilled= "
						+ totalMaterialBilled + ", totalCost= "
						+ totalMaterialCost + ", " + "daily cost= "
						+ dailyMaterialCost);

				toolstable.setItems(loadItem(toolsList)); // loading selected
															// items into the
															// table
	...
	public void removeItem() throws SQLException {
		...
		try {
			...
			// updating daily and ascending totals, daily material cost(raw
			// cost)
			...
			dailyTotal.setText(String.valueOf(dailyCost));
			actualRawCost = Math.round((actualRawCost - (t.getAmount() * t
					.getCost())) * 100.0) / 100.0;
			...
			System.out.println("Removed item: " + t.getName() + ", amount= "
					+ t.getAmount() + ", cost= " + t.getCost()
					+ ", totalCost= " + (t.getAmount() * t.getCost()) + ", "
	...
	private ObservableList<EmployeeHours> loadWorker(List<EmployeeHours> empl) {
		ObservableList<EmployeeHours> worker = FXCollections
				.observableArrayList();
	...
	private ObservableList<String> hours() {
		...
		time.addAll("1:00", "1:30", "2:00", "2:30", "3:00", "3:15", "3:30",
				"3:50", "4:00", "4:15", "4:20", "4:30", "4:50", "5:00", "5:30",
				"6:00", "6:30", "6:50", "7:00", "7:15", "7:30", "7:45", "8:00",
	...
	private double getHours(String in, String out) throws ParseException {
		...
		hours = Math
				.round((Math.abs(date2.getTime() - date1.getTime()) / 3600000.0) * 100.0) / 100.0;
	...
	private void askAnotherWokOrder(ActionEvent event) throws IOException {
		// after saving workorder user will be asked if he wants to enter new
		// Workorder
		if (ConfirmationBox.display("",
				"Would you like to enter another workorder?")) {
	...
	/**
	 * Checks if the job is started, if not program enters start date. If job
	 * completed is selected program gets the date and marks selected date as
	 * finished
	 * 
	 * @throws SQLException
	 * @throws ParseException
	...
	private void checkJobstartedOrFinished() throws SQLException,
		...
		} else if (datechooser.getValue() != null
				&& !connection.isJobStarted(jobNumber)) { // it is the date when
															// job is started
